...
**Plans**: 4 plans (planned 2026-05-02)
- [x] 08-01-PLAN.md — Alias DAL foundation: `trip_type_aliases` table + 26-row seed migration; `forecasts` table drop migration; `src/lib/db/aliases.ts` DAL with ALIAS_JOIN_SQL/CANONICAL_TRIP_TYPE_EXPR fragments; explorer/compare/trends DAL queries made alias-aware (Wave 1 — ALI-01, ALI-02, ALI-05, RTR-03)
- [x] 08-02-PLAN.md — Home page + admin: `src/lib/db/queries/home.ts` + new `/` loader + page; `src/lib/auth/admin.ts` HMAC-signed cookie; `/admin/trip-types` route + login route; NewLabelBadge / HomeSectionCard / BoatBarRow components; per-section bar normalization (Wave 2 — HOME-01..05, ALI-03, ALI-04)
- [x] 08-03-PLAN.md — v1 retirement + redirects: delete `/picker`, `/trends`, `src/lib/forecast/`, calendar heatmap, forecast scripts, allowlist lint, v1-only tests; hooks.server.ts 301 redirects + admin gate hoist; scheduler trim; SLA alert simplification; /about copy rewrite (Wave 2 — RTR-01..09, RDR-01, RDR-02)
- [x] 08-04-PLAN.md — /compare typeahead + polish + theme + chart axis + granularity: theme cookie SSR + ThemeToggle + dark CSS; `+error.svelte`; loading skeletons; empty-state variants; descriptive titles; ECharts theme integration; chart x-axis category→time; GranularitySelector; urlState.ts granularity extension; range-switch reset; /compare datalist typeahead (Wave 3 — POL-01..05, THM-01..03, CMP-01, CMP-02, GRN-01, GRN-02, AXS-01)
**UI hint**: yes
